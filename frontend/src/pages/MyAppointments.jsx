...
const MyAppointments = () => {
  ...
  const payAppointment = async (appointmentId) =>{
    try {
      const {data} = await axios.post('http://localhost:4000/api/user/pay-appointment', {appointmentId}, {headers:{token}})
      if(data.success) {
        toast.success(data.message)
        getUserAppointmnets()
        getDoctorsData()
          ...
          <div className="appointment-card" key={index}>
            ...
            <div className="appt-details">
              ...
              <p className="datetime">
                Date & Time: <br/>{item.slotDate}  | {item.slotTime}
              </p>
            </div>
            <div className="appt-actions">
              
                  { !item.cancelled && !item.payment && <button  onClick={() => payAppointment(item._id)}  className="pay-btn" >
                    Pay here
                  </button> }
                  { !item.cancelled && !item.payment && <button onClick={()=>cancelAppointment(item._id)} className="cancel-btn" >
                    Cancel appointment
                  </button>}
                  {item.cancelled && !item.payment && <button className="cancelled-btn">Appointment cancelled</button>}
                  {!item.cancelled && item.payment && <button className="paid-btn">Paid</button>}
            </div>
          </div>
          )
})
